...
class Filter {
    constructor(props) {
        ...
        this.resetBtnString = `<div reset-filter-btn class="flex-all icon-c cursor-pointer position-absolute reset-filter">
                              ${SVG_string["reset"]}
                        </div>`;

        this.appendContainer = () => {
            if(this.previousNodeContainer) {
                this.parentNode.replaceChild(this.containerNode, this.previousNodeContainer);
            } else {
        ...
        this.onResetClick = () => {
            this.availableOptions = this.availableOptions.map(item => ({...item, value: undefined}));
            this.render();
        }

        this.resetBtnSetup = () => {
            if(this.containerNode) {
                const resetNode = this.containerNode.querySelector("[reset-filter-btn]");
                if(resetNode) {
...
class FilterGenericExpandable extends Filter {
    constructor(props) {
        ...
        this.removeFilterItem = (item, index) => {
            this.availableOptions = this.availableOptions.map((_item, _index) => _item.key === item.key ? {..._item, value: false} : _item);
            this.render();
        }
        ...
        this.onConfirmModal = (modal) => {
            ...
            this.availableOptions = this.availableOptions.map(item => ({...item,
        ...
        this.onModalOpen = () => {
            const modal = new ConfirmationModal(this.onConfirmModal,this.onCancel, "Select options");
        ...
        this.render = () => {
            this.containerNode = stringToHTML(`            
                <div class="filter-item" filter-key="${this.filterKey}" >
                      <div class="filter-name" filter-name>${this.title}: </div>
                      <div class="flex-start flex-wrap" filter-sub-items>
                           ${this.availableOptions.reduce((prevVal, currVal, index) => {
                               if(!currVal.value) {
                                   return prevVal;
                               }
                           ...
                           ${this.availableOptions.every(item => !item.value) ? "<p> Please select. </p>" : ""}
                           
                          <div expand-list-btn="${this.filterKey}" class="flex-all icon-c cursor-pointer position-absolute arrow-expand-c">
            ...
            this.parentNode.querySelector(`[expand-list-btn="${this.filterKey}"]`).addEventListener("click", (event) => {
                this.onModalOpen();
            });

            this.availableOptions.forEach((item, index) => {
                const elementNode = document.getElementById(`delete-item-filter-${this.filterKey}-${item.key}`);
                elementNode && elementNode.addEventListener("click", (event) => {
                    this.removeFilterItem(item, index);
                });
            });
...
class FilterCheckBox extends Filter {
    constructor(props) {
        ...
        this.render = () => {
            ...
            this.itemsContainerNode = this.containerNode.querySelector(`[filter-sub-items]`);
...
class FilterRange extends FilterCheckBox {
    constructor(props) {
        ...
        this.onResetClick = () => {
            this.availableOptions = this.availableOptions.map(item => ({...item, value: 0}));
            this.render();
        }
    }
}
...
class LocationFilter extends FilterGenericExpandable {
    constructor(props) {
        ...
        this.onConfirmModal = (modal) => {
            ...
            console.log(JSON.stringify(modal.getMapResults()))
            if(isNaN(lat) || isNaN(lon)) {
                this.availableOptions[0].value = undefined;
                this.availableOptions[1].value = undefined;
            } else {
        ...
        this.onModalOpen = () => {
            const modal = new ConfirmationModal(this.onConfirmModal,this.onCancel, "Choose location");
            /** pass the list of available options marked with one is selected */
            modal.createMapLocation(this.availableOptions[0].value, this.availableOptions[1].value);
            modal.showModal();
        }
            ...
            this.containerNode = stringToHTML(`            
                <div class="filter-item" filter-key="${this.filterKey}" >
                      ...
                      <div class="flex-start flex-wrap" filter-sub-items>
                           ...
                           ${this.availableOptions.some(item => isNaN(item.value)) ? "<p> Please select. </p>" : `<p>Lat ${this.availableOptions[0].value}, Long ${this.availableOptions[1].value}</p>`}
...
class SearchPage {
    constructor() {
        this.containerNode = document.getElementById(`filter-edit-c`);

        this.selectors = {
            FILTER_TOGGLE: `#search-results [filter-btn]`
        }
        ...
        this.stateFilter = new FilterGenericExpandable({
            title: "State",
            filterKey: "state",
            availableOptions: [{label: "OH", key: `state-${"OH"}`, value: true}, {label: "CA", key: `state-${"CA"}`}],
            parentNode: this.containerNode
        });
        this.stateFilter.render();

        this.circumFilter = new FilterCheckBox({
            title: "Accident circumstances",
            filterKey: "accident-circumstance",
            availableOptions: [
                {label: "Roundabout", key: `accident-circumstance-${"Give way"}`, value: true},
                {label: "Start", key: `Period-${"st-3"}`, value: "2021-02-03"},
                {label: "Give way", key: `accident-circumstance-${"Roundabout"}`}],
            parentNode: this.containerNode
        });
        this.circumFilter.render();


        this.tempFilter = new FilterRange({
            title: "Temperature",
            filterKey: "Temperature",
            availableOptions: [
                {label: "Max", key: `Temperature-${"min"}`, value: 20},
                {label: "Min", key: `Temperature-${"max"}`, value: 30}
            ],
            parentNode: this.containerNode
        });
        this.tempFilter.render();

        this.periodFilter = new DateFilter({
            title: "Period",
            filterKey: "Period",
            availableOptions: [
                {label: "Start", key: `Period-${"st"}`, value: "2021-02-02"},

                {label: "End", key: `Period-${"end"}`, value: "2021-02-02"}
            ],
            parentNode: this.containerNode
        });
        ...
        this.locationFilter = new LocationFilter({
            ...
            filterKey: "Location",
            availableOptions: [
                {label: "Lat", key: `Location-${"Lat"}`, value: 3},
                {label: "Long", key: `Location-${"Long"}`, value: 3}
            ],
            parentNode: this.containerNode
        });
        this.locationFilter.render();

        document.querySelector(this.selectors.FILTER_TOGGLE).addEventListener("click", () => {
